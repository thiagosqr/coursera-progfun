...
class FunSetSuite extends FunSuite {
  ...
  test("filter of elements") {
    new TestSets {
      ...
      val s = filter(union(s1, s2), x => x > 0)
  ...
  test("exists element") {
    new TestSets {
      ...
      assert(exists(singletonSet(1), x => x > 0), "exists 1")
      assert(exists(union(s1,s3), x => x < 2 || x > 2), "exists 1 or 3")
  ...
  test("forall elements") {
    new TestSets {
      ...
      assert(forall(s, x => x > 0) , "all elements are bigger than 0")
